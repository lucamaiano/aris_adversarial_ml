...
class RobustnessMetrics:
    def __init__(self, classifier, predictions, x_test, x_test_adv, y_test, y_target, targeted, dataset, save_dir) -> None:
    ...
    def misclassification_rate(self):
        ...
        for i in range(len(self.x_test_adv)):
            if self.successful(adv_softmax_preds=self.softmax[i], nature_true_preds=self.y_test[i],
                                targeted_preds=self.y_target[i], target_flag=self.target_flag):
                cnt += 1
    ...
    def avg_lp_distortion(self):
        ...
        for i in range(len(self.x_test_adv)):
            if self.successful(adv_softmax_preds=self.softmax[i], nature_true_preds=self.y_test[i],
                ...
                dist_l2 += np.linalg.norm(np.reshape(self.x_test[i] - self.x_test_adv[i], -1), ord=2)
                dist_li += np.linalg.norm(np.reshape(self.x_test[i] - self.x_test_adv[i], -1), ord=np.inf)
    ...
    def avg_SSIM(self):
        ori_r_channel = np.transpose(np.round(self.x_test * 255), (0, 2, 3, 1)).astype(dtype=np.float32)
        adv_r_channel = np.transpose(np.round(self.x_test_adv * 255), (0, 2, 3, 1)).astype(dtype=np.float32)
        ...
        for i in range(len(self.x_test_adv)):
            if self.successful(adv_softmax_preds=self.softmax[i], nature_true_preds=self.y_test[i],
                                targeted_preds=self.y_target[i], target_flag=self.target_flag):
                cnt += 1
                totalSSIM += SSIM(im1=ori_r_channel[i], im2=adv_r_channel[i], data_range=255, channel_axis=2)
        ...
        if self.target_flag is True:
            for i in range(len(self.x_test_adv)):
                if np.argmax(self.softmax[i]) == np.argmax(self.y_target[i]):
                    ...
                    gb_sample = self.gaussian_blur_transform(AdvSample=adv_sample, radius=radius, oriDataset=self.dataset)
        ...
        if self.target_flag is True:
            for i in range(len(self.x_test_adv)):
                if np.argmax(self.softmax[i]) == np.argmax(self.y_target[i]):
                    ...
                    ic_sample = self.image_compress_transform(IndexAdv=i, AdvSample=adv_sample, dir_name=self.save_dir, quality=quality,
    ...
    def image_compress_transform(self, IndexAdv, AdvSample, dir_name, quality, oriDataset):
        if oriDataset.upper() == 'CIFAR10':
            assert AdvSample.shape == (3, 32, 32)
            sample = np.transpose(np.round(AdvSample * 255), (1, 2, 0))
            image = Image.fromarray(np.uint8(sample))
