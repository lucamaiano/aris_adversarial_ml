...
def evaluate_model(classifier, x, y, use_art, logger, attacked=False):
    if use_art:
        predictions = classifier.predict(x)
    else:
        x = [torch.from_numpy(x_tensor) for x_tensor in x]
    ...
    if attacked:
        logger.info(
            "Accuracy on attacked test examples: {:.3f}%".format(accuracy * 100)
        )
...
def save_model(model: torch.nn.Module, target_dir: str, model_name: str, use_art: bool, logger):
    ...
    if type(model) == art.estimators.classification.pytorch.PyTorchClassifier:
        art.config.ART_DATA_PATH = target_dir
