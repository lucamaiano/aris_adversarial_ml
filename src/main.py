...
def train(conf: DictConfig) -> None:
    ...
    INPUT_SHAPE = conf.dataset.input_shape
    NB_CLASSES = conf.dataset.nb_classes
    
    # Load data
    (x_train, y_train), (x_test, y_test), min_pixel_value, max_pixel_value = data_setup.load_data(conf.dataset.name)
    ...
    if not conf.train.load_pretrained:
        ...
        classifier = engine.test(
            model=model,
            model_path=to_absolute_path(Path("models", conf.train.name, f"{conf.train.model_path}.model")),
            x_train=x_train,
            y_train=y_train,
            nb_classes=NB_CLASSES,
            clip_values=(min_pixel_value, max_pixel_value),
    ...
    attack = engine.attack(attack_name=conf.attack.name, x=x_test, y=y_test, nb_classes=NB_CLASSES, classifier=classifier_adv, logger=log, targeted=conf.attack.targeted)
    
    # Evaluate the attacked model metrics
    predictions_adv, accuracy_adv = utils.evaluate_model(classifier=classifier_adv, x=attack.x_adv, y=y_test, logger=log, attacked=True)
